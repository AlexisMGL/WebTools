...
function findMaxDifference(time, values) {
    ...
    for (let i = 0; i < time.length; i++) {
        ...
        let targetTime = time[i] + 20; // Ajouter 20 secondes à l'instant actuel
...
function print_ff(log, t1, t2, head) {
    let fieldset = document.createElement("fieldset")
    let fieldsetlab = document.createElement("fieldset")
...
async function load(e) {
    ...
    document.title = "Datalab: " + file.name
...
function reset() {
    ...
    document.title = "AerialMetric Datalab"
...
function processAMData(files) {
    ...
    // Fonction pour lire un fichier et retourner son contenu en ArrayBuffer
